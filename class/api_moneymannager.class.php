 ...
 */
class MoneyMannager extends DolibarrApi
{
	/**
	...
	{
		$list = array();
		dol_syslog("Apimoneymannager::index rights=".print_r(DolibarrApiAccess::$user->rights->moneymannager,true));
		if (!DolibarrApiAccess::$user->rights->moneymannager->read) {
		...
		if ($result) {
			...
			for ($i = 0; $i < $min; $i++) {
				...
				$account = new CustomAccount($this->db);
	...
	{
		if (!DolibarrApiAccess::$user->rights->banque->transfer) {
			throw new RestException(401);
		}
	...
	{
		if (!DolibarrApiAccess::$user->rights->moneymannager->configurer) {
			throw new RestException(401);
		}
	...
	public function getLines($id, $sqlfilters = '')
	{
		...
		if (!DolibarrApiAccess::$user->rights->moneymannager->lire) {
			throw new RestException(401);
		}
	...
	{
		if (!DolibarrApiAccess::$user->rights->moneymannager->modifier) {
			throw new RestException(401);
		}
	...
	/**
	 * Create a new payment various
	 * @param int $id ID of account
	 * @param string $date Payment date (timestamp) {@from body} {@type timestamp}	 
	 * @param string $label Label {@from body}
	 * @param float $amount Amount (may be 0) {@from body}
	 * @param string $method Payment mode (TYP,VIR,PRE,LIQ,VAD,CB,CHQ...) {@from body}
	 * @param int $sense Sense {@from body}
	 * @param string $datev Payment date value (timestamp) {@from body} {@type timestamp}
	 * @param int $category Category
	 * @param string $transmitter Transmitter {@from body}
	 * @param string $voucher_number Voucher number {@from body}
	 * @param string $bank Bank {@from body}
	 * @param string $accountancycode Accountancy code {@from body}
	 * @param string $subaccount Subaccount {@from body}
	 * @param int $project_id Project ID {@from body}
	 ...
	 */
	public function addPayment($id, $date,  $label, $amount, $method, $sense = 0,$datev='', $categoryid = 0, $transmitter = '', $voucher_number = '',$bank ='', $accountancycode = '', $subaccount = '', $project_id = 0)
	{
		if (!DolibarrApiAccess::$user->rights->moneymannager->read) {
			throw new RestException(401);
		}
		...
		if (!DolibarrApiAccess::$user->rights->moneymannager->payment) {
			throw new RestException(401);
		}
		dol_include_once('/compta/bank/class/paymentvarious.class.php');

		$object = new PaymentVarious($this->db);

		$object->ref = ''; // TODO
		$object->fk_account = $account->id ;
		$object->datev = $datev??$date;
		$object->datep = $date;
		$object->amount = price2num($amount);
		$object->label = $label;
		
		
		$object->type_payment = dol_getIdFromCode($this->db,$method, 'c_paiement', 'code', 'id', 1);
	    $object->num_payment = $voucher_number;
		$object->chqemetteur = $transmitter;
		$object->chqbank = $bank ;
		$object->fk_user_author = DolibarrApiAccess::$user->id;
		$object->fk_project = $project_id;
		$object->accountancy_code = $accountancycode;
		$object->subledger_account = $subaccount;	 
		$object->category_transaction = $categoryid;
		$object->sens = $sense;

		$res = $object->create(DolibarrApiAccess::$user);
		if ($res < 0) {
			throw new RestException(503, 'Error when creating payment: '.$object->error);
		}
		return $object->id;
